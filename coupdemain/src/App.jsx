...
const CATEGORIES = [
  { id: "jardin", label: "Jardinage", icon: Sprout, hue: "#3EC17A" },
  { id: "animaux", label: "Garde d'animaux", icon: Dog, hue: "#FF8A3D" },
  { id: "info", label: "Aide informatique", icon: Laptop2, hue: "#3EC1FF" },
  { id: "courses", label: "Courses", icon: ShoppingBag, hue: "#FF5D8F" },
  { id: "menage", label: "Ménage", icon: Brush, hue: "#8B5CF6" },
  { id: "demenagement", label: "Déménagement léger", icon: Package, hue: "#A78BFA" },
  { id: "cours", label: "Cours particuliers", icon: BookOpen, hue: "#3EC1FF" },
  { id: "voiture", label: "Lavage de voiture", icon: Car, hue: "#22C3D6" },
  { id: "bricolage", label: "Petit bricolage", icon: Hammer, hue: "#FF8A3D" },
  { id: "babysitting", label: "Baby-sitting", icon: BabySit, hue: "#FFD23F" },
  { id: "promenade", label: "Balade compagnie", icon: PawPrint, hue: "#3EC17A" },
  { id: "peinture", label: "Peinture, déco", icon: Paintbrush, hue: "#FF5D8F" },
  { id: "hiver", label: "Déneigement", icon: Snowflake, hue: "#3EC1FF" },
  { id: "autre", label: "Autre", icon: Sparkles, hue: "#8B5CF6" },
];
...
function hardShadow(px = 4) { return `${px}px ${px}px 0 ${INK}`; }

function StarRow({ value }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 3, background: "#FFD23F", border: `1.5px solid ${INK}`, borderRadius: 20, padding: "2px 8px 2px 6px" }}>
...
export default function App() {
  ...
  function pay() {
    setStage("done");
    setMessages([{ from: "them", text: `Hey ! Merci pour la résa 🙌 je peux venir ${["demain", "ce week-end", "mercredi"][selected.id % 3]}, ça te va ?` }]);
  }
  ...
  return (
    <div style={{ fontFamily: "'Nunito', system-ui, sans-serif", background: BG, minHeight: "100%", color: INK }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ padding: "20px 18px 16px", background: "#8B5CF6", borderBottom: `3px solid ${INK}` }}>
        <div className="disp" style={{ fontSize: 26, fontWeight: 700, color: "#fff", textShadow: `2px 2px 0 ${INK}` }}>
          CoupDeMain 🙌
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          {[["client", "Je cherche 🔍"], ["jeune", "Je propose 💪"]].map(([val, label]) => (
            <button key={val} className="press" onClick={() => { setRole(val); setStage("browse"); }}
              style={{
                padding: "9px 14px", borderRadius: 14, border: `2px solid ${INK}`, fontSize: 12.5, fontWeight: 700,
                background: role === val ? "#FFD23F" : "#fff",
      ...
      {role === "client" && stage === "browse" && (
        <div style={{ padding: "16px 18px" }}>
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 12 }}>
            <button className="chip" onClick={() => setActiveCat(null)} style={chipStyle(activeCat === null, "#FFD23F")}>Tout</button>
            {CATEGORIES.map((c) => (
              <button key={c.id} className="chip" onClick={() => setActiveCat(c.id)} style={chipStyle(activeCat === c.id, c.hue)}>
          ...
          <div style={{ background: CARD, border: `2px dashed ${INK}`, borderRadius: 14, padding: "13px 14px", marginBottom: 16 }}>
            <div className="disp" style={{ fontSize: 14, fontWeight: 600, marginBottom: 3 }}>Un truc bizarre à demander ? 🤔</div>
            ...
            {!customSent ? (
              <div style={{ display: "flex", gap: 7 }}>
                <input value={customRequest} onChange={(e) => setCustomRequest(e.target.value)}
                  ...
                  style={{ flex: 1, border: `2px solid ${INK}`, borderRadius: 10, padding: "9px 11px", fontSize: 12.5 }} />
                <button className="press" onClick={() => customRequest.trim() && setCustomSent(true)}
                  style={{ background: "#FF5D8F", color: "#fff", border: `2px solid ${INK}`, boxShadow: hardShadow(3), borderRadius: 10, padding: "0 14px", fontSize: 12, fontWeight: 700 }}>
                  Go
                </button>
              </div>
            ) : (
              <div style={{ fontSize: 12.5, color: INK, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
                <Check size={15} /> Envoyé ! On te tient au courant 🎉
              </div>
            )}
          </div>
            ...
            {filtered.map((p) => {
              ...
              return (
                <div key={p.id} className="press" onClick={() => openProfile(p)} style={{
                  background: CARD, border: `2.5px solid ${INK}`, borderRadius: 16, padding: "14px 15px",
                  boxShadow: hardShadow(4), transform: `rotate(${p.rotate}deg)`,
                }}>
...
function chipStyle(active, hue) {
  return {
    ...
    padding: "8px 12px", borderRadius: 20, border: `2px solid ${INK}`, fontSize: 12, fontWeight: 700,
  ...
  return (
    <div style={{ padding: "16px 18px 32px" }}>
      <button className="press" onClick={onBack} style={{
        display: "flex", alignItems: "center", gap: 4, border: `2px solid ${INK}`, background: "#fff",
...
function DetailPanel({ p, onBack, onBook }) {
  return (
    <PanelShell onBack={onBack} title={p.name} emoji="👋">
      ...
      <div style={{ background: CARD, border: `2px solid ${INK}`, borderRadius: 14, padding: "12px 14px", marginTop: 12, fontSize: 13, lineHeight: 1.5, fontWeight: 600 }}>
        {p.bio}
      </div>

      <div className="disp" style={{ fontSize: 15, fontWeight: 600, marginTop: 18, marginBottom: 8 }}>Ce qu'iel propose 🛠️</div>
      {p.skills.map((s) => (
        <div key={s.cat} style={{
          display: "flex", justifyContent: "space-between", alignItems: "center",
          background: catOf(s.cat).hue + "22", border: `2px solid ${INK}`, borderRadius: 14, padding: "10px 12px", marginBottom: 9,
        }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 800 }}>{catOf(s.cat).label}</div>
            <div style={{ fontSize: 11.5, color: "#5C5346", fontWeight: 600 }}>{s.note}</div>
          </div>
          <button className="press" onClick={() => onBook(s)} style={{ border: `2px solid ${INK}`, background: "#FFD23F", fontSize: 12, fontWeight: 800, padding: "8px 12px", borderRadius: 12, whiteSpace: "nowrap", boxShadow: hardShadow(3) }}>
            {s.rate} CHF/h
          </button>
        </div>
      ))}
      ...
      {p.reviewList.map((r, i) => (
        <div key={i} style={{ background: "#fff", border: `2px solid ${INK}`, borderRadius: 12, padding: "10px 12px", marginBottom: 8 }}>
...
function BookingPanel({ p, skill, price, commission, payout, onBack, onConfirm }) {
  return (
    <PanelShell onBack={onBack} title="On confirme ?" emoji="✅">
      ...
      <div style={{ background: "#fff", border: `2px solid ${INK}`, borderRadius: 14, padding: "6px 14px" }}>
      ...
      <button className="press" onClick={onConfirm} style={{ width: "100%", marginTop: 18, background: "#8B5CF6", color: "#fff", border: `2.5px solid ${INK}`, boxShadow: hardShadow(4), padding: "13px", borderRadius: 14, fontWeight: 800, fontSize: 14, display: "flex", justifyContent: "center", alignItems: "center", gap: 6 }}>
        Direction le paiement <ArrowRight size={15} />
      </button>
    </PanelShell>
  );
}
    ...
    <PanelShell onBack={onBack} title="Paiement" emoji="💳">
      ...
      {[["twint", "Twint"], ["card", "Carte bancaire"]].map(([v, l]) => (
        <button key={v} className="press" onClick={() => setMethod(v)} style={{
          ...
          border: `2px solid ${INK}`, background: method === v ? "#FFD23F" : "#fff",
      ...
      <button className="press" onClick={onPay} style={{ width: "100%", marginTop: 8, background: "#3EC17A", color: "#fff", border: `2.5px solid ${INK}`, boxShadow: hardShadow(4), padding: "13px", borderRadius: 14, fontWeight: 800, fontSize: 14 }}>
      ...
      <div style={{ minHeight: 100, marginBottom: 12 }}>
        {messages.map((m, i) => (
          <div key={i} style={{
            ...
            background: m.from === "me" ? "#8B5CF6" : "#fff", color: m.from === "me" ? "#fff" : INK,
            border: `2px solid ${INK}`, padding: "9px 12px", borderRadius: 14, fontSize: 12.5, fontWeight: 600,
          }}>{m.text}</div>
        ))}
      </div>
      <div style={{ display: "flex", gap: 7 }}>
        <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Écrire un message..."
          ...
          style={{ flex: 1, border: `2px solid ${INK}`, borderRadius: 12, padding: "10px 12px", fontSize: 12.5 }} />
        <button className="press" onClick={onSend} style={{ background: "#FFD23F", border: `2px solid ${INK}`, boxShadow: hardShadow(3), borderRadius: 12, width: 40, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Send size={14} />
        </button>
      </div>
    </PanelShell>
...
function JeuneProfile({ signupAge, setSignupAge }) {
  ...
  return (
    <PanelShell onBack={() => {}} title="Mon profil" emoji="✨">
      <div style={{ marginBottom: 14 }}>
        ...
        <input type="number" value={signupAge} onChange={(e) => setSignupAge(e.target.value)}
          placeholder="Ex : 16" style={{ width: "100%", border: `2px solid ${INK}`, borderRadius: 12, padding: "10px 12px", fontSize: 13, marginTop: 5 }} />
      </div>

      {isMinor && (
        <div style={{ background: "#FFE8A3", border: `2px solid ${INK}`, borderRadius: 12, padding: "11px 13px", fontSize: 12.5, display: "flex", gap: 8, marginBottom: 14, fontWeight: 600, lineHeight: 1.5 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <textarea placeholder="Motivé, ponctuel, disponible les mercredis et week-ends..."
          style={{ width: "100%", border: `2px solid ${INK}`, borderRadius: 12, padding: "10px 12px", fontSize: 13, marginTop: 5, minHeight: 64, fontFamily: "inherit" }} />
      </div>

      <div style={{ fontSize: 12, fontWeight: 800, marginBottom: 8 }}>Tes services</div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 16 }}>
        {CATEGORIES.map((c) => (
          <button key={c.id} className="chip" style={{ display: "flex", alignItems: "center", gap: 5, border: `2px solid ${INK}`, background: "#fff", borderRadius: 20, padding: "7px 11px", fontSize: 11.5, fontWeight: 700 }}>
            <c.icon size={13} /> {c.label}
          </button>
        ))}
      </div>

      <div style={{ background: "#fff", border: `2px solid ${INK}`, borderRadius: 14, padding: "12px 14px" }}>
      ...
      <button className="press" disabled={isMinor} style={{
        width: "100%", marginTop: 16, background: isMinor ? "#E4DCC8" : "#FF5D8F", border: `2.5px solid ${INK}`,
